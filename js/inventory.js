let deleteTargetIndex = -1;
        ...
        function renderHistory() {
            ...
            if(state.transactions.length === 0) { 
                tbody.innerHTML = `<tr><td colspan="6" class="p-8 text-center text-slate-400 font-semibold text-xs">Belum ada riwayat transaksi.</td></tr>`; return; 
            }
            state.transactions.forEach(tx => {
                const tr = document.createElement('tr');
                tr.className = "hover:bg-slate-50/50 transition-colors";
                tr.innerHTML = `
                    <td class="p-4 font-mono font-semibold text-xs text-brand-600">${tx.invoice}</td>
                    <td class="p-4 text-xs font-bold text-slate-500">${tx.datetime}</td>
                    <td class="p-4 text-right font-black text-slate-700">${formatRupiah(tx.total)}</td>
                    <td class="p-4 text-right font-semibold text-slate-500">${formatRupiah(tx.cashPaid)}</td>
                    <td class="p-4 text-right font-semibold text-slate-500">${formatRupiah(tx.change)}</td>
                    <td class="p-4 text-center">
                        <button onclick='reprintTx("${tx.invoice}")' class="px-3 py-1.5 bg-slate-100 hover:bg-brand-50 hover:text-brand-700 rounded-lg text-xs font-semibold text-slate-600 transition-colors">
        ...
        function reprintTx(invoiceId) {
            const tx = state.transactions.find(t => t.invoice === invoiceId);
        ...
        function saveRestock(e) {
            ...
            const localSave = () => {
                ...
                saveToLocalCache(); renderPOSProducts(); renderInventory(); updateDashboardStats();
                if (!document.getElementById('modal-low-stock').classList.contains('hidden')) { renderLowStockList(); }
                closeRestockModal(); return true;
            };

            if (isCloudMode) {
                toggleLoadingOverlay(true);
                google.script.run
                    .withSuccessHandler(() => { toggleLoadingOverlay(false); localSave(); showToast(`Stok ${p.name} berhasil ditambah +${inputQty} (Cloud)!`, 'success'); })
                    .withFailureHandler((err) => { toggleLoadingOverlay(false); showToast('Gagal update ke Google Sheets: ' + err.message, 'error'); })
                    .saveProduct(updatedProduct);
            } else {
                localSave(); showToast(`Stok ${p.name} berhasil ditambah +${inputQty} secara lokal.`, 'success');
            }
        }
        ...
        function setFormImage(src) {
            ...
            if (src) { preview.src = src; preview.classList.remove('hidden'); placeholder.classList.add('hidden'); } 
        ...
        function handleImageUpload(e) {
            ...
            if (file) { const r = new FileReader(); r.onload = function(ev) { setFormImage(ev.target.result); document.getElementById('form-product-image-url').value = ''; }; r.readAsDataURL(file); }
        }
        ...
        // --- UPDATE BARU: DETEKSI KAMERA SCAN SKU CERDAS ---
        ...
        function stopSkuScanner() {
            // FIX: Sembunyikan modal duluan biar tombol batal gak macet
        ...
        function closeProductForm() { stopCamera(); document.getElementById('modal-product-form').classList.add('hidden'); }
        ...
        function saveProduct(e) {
            ...
            const localSave = () => {
                if (idx > -1) { state.products[idx] = { ...state.products[idx], ...p }; } 
                else {
                    if(state.products.some(pr => pr.sku === p.sku)) { showToast('SKU sudah terdaftar!', 'error'); return false; }
                    state.products.push(p);
                }
                saveToLocalCache(); renderPOSProducts(); renderInventory(); updateDashboardStats(); closeProductForm(); return true;
            };

            if (isCloudMode) {
                toggleLoadingOverlay(true);
                google.script.run.withSuccessHandler(() => { toggleLoadingOverlay(false); localSave(); showToast('Tersimpan di Cloud Sheets!', 'success'); }).saveProduct(p);
            } else { if(localSave()) showToast('Tersimpan secara lokal.', 'success'); }
        }
        ...
        function deleteProduct(index) {
            ...
            document.getElementById('btn-confirm-delete-action').onclick = () => {
                ...
                const localDel = () => { 
                    state.products.splice(deleteTargetIndex, 1); saveToLocalCache(); document.getElementById('modal-confirm-delete').classList.add('hidden'); renderPOSProducts(); renderInventory(); updateDashboardStats(); 
                };
                if (isCloudMode) {
                    toggleLoadingOverlay(true);
                    google.script.run.withSuccessHandler(() => { toggleLoadingOverlay(false); localDel(); showToast('Terhapus di Cloud!', 'info'); }).deleteProduct(target.sku);
                } else { localDel(); showToast('Terhapus secara lokal.', 'info'); }
            };
        }

        function closeDeleteModal() { document.getElementById('modal-confirm-delete').classList.add('hidden'); }
