function filterCategory(category) {
        ...
        function addToCart(sku) {
            ...
            if (existingCartItem) {
                if (existingCartItem.qty + 1 > product.stock) { showToast('Stok tidak mencukupi.', 'error'); return; }
        ...
        function renderCart() {
            ...
            views.forEach(view => {
                ...
                if (state.cart.length === 0) {
                    ...
                    view.payBtn.disabled = true; view.payBtn.className = "col-span-2 w-full py-4 bg-slate-100 text-slate-400 rounded-2xl font-bold transition-all flex items-center justify-center gap-2 cursor-not-allowed";
                    view.subtotal.innerText = formatRupiah(0); view.total.innerText = formatRupiah(0);
                } else {
                    if (view.empty) view.empty.classList.add('hidden');
                    if (view.list) view.list.classList.remove('hidden');
                    view.payBtn.disabled = false; view.payBtn.className = "col-span-2 w-full py-4 bg-gradient-to-r from-brand-600 to-emerald-500 hover:from-brand-700 hover:to-emerald-600 text-white rounded-2xl font-black transition-all flex items-center justify-center gap-2 shadow-lg shadow-brand-500/20 cursor-pointer";
                    ...
                    view.subtotal.innerText = formatRupiah(subtotal); view.total.innerText = formatRupiah(subtotal);
                }
            });
        }
        ...
        function openPaymentModal() {
            ...
            fastOptions.sort((a,b) => a - b).slice(0, 3).forEach(opt => {
                ...
                btn.className = "p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold hover:bg-brand-50 text-slate-600";
        ...
        function calculateChange() {
            ...
            if(change >= 0) {
                changeEl.innerText = formatRupiah(change); changeEl.className = "text-lg font-black text-brand-600";
                submitBtn.disabled = false; submitBtn.className = "w-full py-3.5 bg-gradient-to-r from-brand-600 to-emerald-500 text-white rounded-2xl font-black shadow-lg cursor-pointer";
            } else {
                changeEl.innerText = 'Uang kurang'; changeEl.className = "text-sm font-bold text-rose-500";
                submitBtn.disabled = true; submitBtn.className = "w-full py-3.5 bg-slate-100 text-slate-400 rounded-2xl font-black cursor-not-allowed";
            }
        }
        ...
        function processCheckout() {
            ...
            const txRecord = { invoice: invoiceId, datetime: datetimeStr, total: currentGrandTotal, cashPaid: inputCash, change: change, items: state.cart.map(i => ({ sku: i.sku, name: i.name, qty: i.qty, price: i.price })) };
            ...
            if (isCloudMode) {
                ...
                google.script.run.withSuccessHandler(() => {
                    ...
                    showToast('Transaksi Berhasil di Cloud Sheets!', 'success');
                }).processTransactionCloud(txRecord);
            } else {
                completeLocalCheckout();
        ...
        function openReceiptModal(tx) {
            ...
            tx.items.forEach(item => {
                ...
                r.innerHTML = `<div><p class="font-bold">${item.name}</p><p class="text-slate-400 text-[9px]">${item.qty} x ${formatRupiah(item.price)}</p></div><span>${formatRupiah(item.price * item.qty)}</span>`;
                list.appendChild(r);
            });
